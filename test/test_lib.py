"""
Tests for GlobalMetabolicNetwork initialization, pruning, and setup pipeline.

Tests the constructor and method behavior across different metabolism types
(KEGG_OG, ecg, KEGG), including expected exceptions for unsupported operations.
"""
...
class TestGlobalMetabolicNetworkInit(unittest.TestCase):
    ...
    def test_invalid_metabolism_raises(self):
        with self.assertRaises(ValueError):
            ne.GlobalMetabolicNetwork("not_a_real_metabolism")


class TestLoadTupleNetwork(unittest.TestCase):
    """Test _load_tuple_network error handling (not covered elsewhere)."""

    def test_single_element_tuple_raises(self):
        with self.assertRaises(ValueError):
            ne._load_tuple_network([([1],)])

    def test_triple_element_tuple_raises(self):
        with self.assertRaises(ValueError):
